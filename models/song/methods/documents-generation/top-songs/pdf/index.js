...
module.exports = (content, options = {}) => {
  return sequelize.continueTransaction(options, transaction => {
    ...
    .then(songs => {
      ...
      document.font('DejaVuSans').fontSize(25).text(`Most Popular Songs Ever`, {align: 'center'}).moveDown(1);

      // Songs
      songs.forEach((song, index) => document.font('DejaVuSans').fontSize(18).text(`${index + 1}) ${generateSongName(song)}`, {align: 'left'}).moveDown(0.5));
...
function generateSongName(song) {
  const artists = song.artists.map(artist => `${artist.firstName} ${artist.lastName} & `).join('').slice(0, -3);
  return `${song.name} (${artists}), listened ${song.listeningStatistics.total} times.`;
}